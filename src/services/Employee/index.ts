...
export const getEmployee = (): Employee[] => {
    const saved = localStorage.getItem(STORAGE_KEY_EMPLOYEE);
    return saved ? JSON.parse(saved) : [];
};

export const saveEmployee = (configs: Employee[]) => {
    localStorage.setItem(STORAGE_KEY_EMPLOYEE, JSON.stringify(configs));
};

// Định nghĩa KEY lưu trữ cho Đơn hàng
